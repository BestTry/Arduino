// The purpose of this code to to allow helicopter cyclic and rudder controls to interface with the Windows OS. Windows recognizes the USB device as a 
...
// simulator or game software. Only the helicopter cyclic and antitorque controls are implemented with one USB cable plugged into the PC for both controls. 
// The helicopter collective and throttle controls were not addressed here. Since they did not need to be repaired re-engineered in the helicopter simulator. 
//
// The Program tests/executes a USB Joystick device driver running on an Arduino Leonardo or Arduino Micro.  The Arduino Joystick class 'Joystick_' 
...
// is seen by Windows as a "Game Controller", or in my case it is the 'cyclic and rudder (anti-torque)' controller for my Helicopter Flight Simulator.
//
// There are two distinct parts of the code. The first part reads the status of the five push-buttons and buttons on the four-way hat switch,  The codes's 
// second part reads the status of the helicopter cyclic potentiomers and the rudder (anti-torque) pot that represents the cammand angle for rotation.
//
// In further detail, the first section of code reads nine push-buttons including four contacts that are a part of the hat-switch. The code reads
...
// pitch, roll and yaw on the helicopter flight simulator mechannical hardware: the'Cyclic' stick and the Anti-torgue peddals.  These are the 
...
// end from the Arduino Micro board and are grounded to the Arduino's ground at the other end.
//
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// TESTING: In order to observe the movement and status of the axis and pushbuttons in Windows: From 'Contol Panel', right click on the Arduino Thumbnail,
//          select 'Game controller settings', select 'Properties', then the 'Arduino Leonardo properties' popup window should appear.
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// *** Note! *** - Code here is designed for the Arduino 'Pro Micro' using the USB translator aboard the Atmel MEGA32U4. Other versions will require pin 
// re-assignments (2020-11-25 Version 3.1 tested on the protoboard breadboard.)  Replaceing the Leonardo with the Arduino 'Pro Micro' significantly reduced 
// micro-controller footprint that is compatible with its larger Leonardo big brother version. Re-assigned the analog inputs pins from ('A0', 'A1', 'A2') to
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// NOTE 1: This code file is for use with Arduino Leonardo Pro Micro only. The Arduino 'UNO' and the like will not work with a personal computer PC 
//         for USB data transmission as a USB divice.  NOTE 2: The 'A0' pin usage to jump start program has been discontinued by commenting out the code.
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
