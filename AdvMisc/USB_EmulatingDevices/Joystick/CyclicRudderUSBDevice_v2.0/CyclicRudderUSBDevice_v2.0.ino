...
//----------------------------------------------------------------------------------------------------------------------------------------------------------
// *** Note! *** - This version is implemented for the Arduino 'Leonardo' and 'Micro'. Other version will require pin re-assignments.
// 2020-11-16 Versio 2.0 Prototype version on protoboard breadboard.  Replace the Arduino Leonardo with the Arduino 'Micro' a significantly reduced 
// real estate micro-controller that is compatible with its larger big brother version.   ----   Tom Ricci
//----------------------------------------------------------------------------------------------------------------------------------------------------------
...
// Looking at connector face on: USB Pin Assignments:      1: 5V    2: Data   3:Data+   4: Mode  5: Gnd / <Shield>
//
...
// Btn Var Name - Pin#          // Digital input pins 2 - 10 are grounded by push-buttons on 'Cyclic' when pressed        --- Pin# ---
const int btn1  =  2;           // Cyclic Button 1 is connected to Arduino pin 2  => Cyclic button position - Trigger     -     2
const int btn2  =  3;           // Cyclic Button 2 is connected to Arduino pin 3  => Cyclic button position - Left Mid    -     3
const int btn3  =  4;           // Cyclic Button 3 is connected to Arduino pin 4  => Cyclic button position - Left Rear   -     4
const int btn4  =  5;           // Cyclic Button 4 is connected to Arduino pin 5  => Cyclic button position - Left Top    -     5
const int btn5  =  6;           // Cyclic Button 5 is connected to Arduino pin 6  => Cyclic button position - Right Top   -     6
const int btn6  =  7;           // Hat Switch up    button is connected to pin 7  -  Hat Switch position direction UP     -     7
const int btn7  =  8;           // Hat Switch right butoon is connected to pin 8  -  Hat Switch position direction RIGHT  -     8
const int btn8  =  9;           // Hat Switch down  button is connected to pin 9  -  Hat Switch position direction DOWN   -     9
const int btn9  = 10;           // Hat Switch left  button is connected to pin 10 -  Hat Switch position direction LEFT   -    10
//                GND           // Two ground labelled (designated) pins on the 'Arduino Micro connected the DB-15 pin #  -     1
//
